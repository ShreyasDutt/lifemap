...
export default function Deck({ cards }) {
  ...
  const [topCardIndex, setTopCardIndex] = useState(null)
  ...
  const bind = useDrag(({ args: [index], active, movement: [mx], direction: [xDir], velocity: [vx] }) => {
    ...
    if (active) setTopCardIndex(index)
  ...
  return (
    <div className={`overflow-hidden relative h-[66vh] w-full flex justify-center items-center ${indie.className}`}>
      {props.map(({ x, y, rot, scale }, i) => (
        <animated.div
          key={i}
          style={{ x, y, zIndex: topCardIndex === i ? 10 : i }}
